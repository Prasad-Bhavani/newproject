    ...
    .controller('ui.rCalendar.CalendarController', ['$scope', '$attrs', '$parse', '$interpolate', '$log', 'dateFilter', 'calendarConfig', function ($scope, $attrs, $parse, $interpolate, $log, dateFilter, calendarConfig) {
        ...
        self.render = function () {
            if (ngModelCtrl.$modelValue) {
                var dateVal=ngModelCtrl.$modelValue.split('-');
                var date = new Date(dateVal[2]+" "+dateVal[0]+" "+dateVal[1]),
                    isValid = !isNaN(date);
    ...
    .directive('monthview', ['dateFilter', function (dateFilter) {
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrls) {
                ...
                scope.select = function (viewDate, title) {
                    var rows = scope.rows;
...
angular.module("template/rcalendar/calendar.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rcalendar/calendar.html",
    ...
    "        </div>-->\n" +
    "        <div class=\"calendar-header col-xs-11\">{{title}}</div>\n" +
...
angular.module("template/rcalendar/month.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("template/rcalendar/month.html",
    ...
    "            </td>\n" +
    "            <td ng-repeat=\"dt in row track by dt.date\" class=\"monthview-dateCell\" ng-disabled=\"dt.events[0].isHoliday\" ng-click=\"dt.events[0].type!=\'Holiday\' && dt.day!=0 && !dt.secondary && select(dt,dt.events[0].title)\"\n" +
    "                ng-class=\"{'text-center':true, 'monthview-current': dt.current&&!dt.selected&&!dt.hasEvent,'monthview-secondary-with-event': dt.secondary&&dt.hasEvent, 'monthview-primary-with-event':!dt.secondary&&dt.hasEvent&&!dt.selected, 'monthview-selected': dt.selected, 'absentcss':dt.events[0].type==\'Absent\', 'sundaycss':dt.day==0}\">\n" +
    "                <div>\n" +
    "                    {{dt.label}}<br /><span ng-repeat=\"dtTitle in dt.events[0].title.split(\'*\')\">{{dtTitle}}<br /></span>\n" +
    "                </div>\n" +
    "            </td>\n" +
    "        </tr>\n" +
    "        </tbody>\n" +
    ...
    "                    <td class=\"event-detail\" ng-click=\"eventSelected({event:event})\"><span ng-repeat=\"dtTitle in event.title.split(\'*\')\">{{dtTitle}}<br /></span></td>\n" +
    "                </tr>\n" +
